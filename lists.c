...
void free_list(struct list_data * pList)
{
  ...
  if (pList->iSize > 0)
    mudlog(CMP, ADMLVL_GOD, TRUE, "List being freed while not empty.");
...
void remove_from_list(void * pContent, struct list_data * pList)
{
  ...
  if ((pRemovedItem = find_in_list(pContent, pList)) == NULL) {
    mudlog(CMP, ADMLVL_GOD, TRUE, "WARNING: Attempting to remove contents that don't exist in list.");
    return;
  }
...
void * merge_iterator(struct iterator_data * pIterator, struct list_data * pList)
{
  ...
  if (pList == NULL) {
    mudlog(NRM, ADMLVL_GOD, TRUE, "WARNING: Attempting to merge iterator to NULL list.");
    pIterator->pList = NULL;
    pIterator->pItem = NULL;
    return NULL;
  }
  if (pList->pFirstItem == NULL) {
    mudlog(NRM, ADMLVL_GOD, TRUE, "WARNING: Attempting to merge iterator to empty list.");
    pIterator->pList = NULL;
    pIterator->pItem = NULL;
    return NULL;
  }
...
void remove_iterator(struct iterator_data * pIterator)
{
  if (pIterator->pList == NULL) {
    mudlog(NRM, ADMLVL_GOD, TRUE, "WARNING: Attempting to remove iterator from NULL list.");
    return;
  }
...
void * simple_list(struct list_data * pList)
{
  ...
  if (!loop || pLastList != pList) {
    if (loop && pLastList != pList)
      mudlog(CMP, ADMLVL_GRGOD, TRUE, "SYSERR: simple_list() forced to reset itself.");
