...
int perform_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if (ch == NULL || dir < 0 || dir >= NUM_OF_DIRS || FIGHTING(ch))
    ...
    send_to_char(ch, "Alas, you cannot go that way...\r\n");
  else if (EXIT_FLAGGED(EXIT(ch, dir), EX_CLOSED) && (GET_ADMLEVEL(ch) < ADMLVL_IMMORT || (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOHASSLE)))) {
...
int ok_pick(struct char_data *ch, obj_vnum keynum, int pickproof, int dclock, int scmd)
{
  ...
  skill_lvl = get_skill_value(ch, SK_LOCKPICKING);
  ...
  if ((obj) || (door >= 0)) {
    ...
    else if (!(DOOR_IS_UNLOCKED(ch, obj, door)) && IS_SET(flags_door[subcmd], NEED_UNLOCKED) && ((!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_AUTOKEY))) && (!has_key(ch, keynum)) )
    {
      send_to_char(ch, "It is locked, and you do not have the key!\r\n");
             ...
             (GET_ADMLEVEL(ch) < ADMLVL_IMMORT || (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOHASSLE))))
