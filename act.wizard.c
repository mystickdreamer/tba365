...
ACMD(do_switch) {
    ...
    else if ((GET_LEVEL(ch) < ADMLVL_IMPL) && !IS_NPC(victim))
        send_to_char(ch, "You are not holy enough to use their body.\r\n");
    else if (GET_LEVEL(ch) < ADMLVL_GRGOD && ROOM_FLAGGED(IN_ROOM(victim), ROOM_GODROOM))
        send_to_char(ch, "You are not godly enough to use that room!\r\n");
...
ACMD(do_load) {
    ...
    if (is_abbrev(buf, "mob")) {
        ...
        if (GET_LEVEL(ch) < ADMLVL_GRGOD && !can_edit_zone(ch, world[IN_ROOM(ch)].zone)) {
...
ACMD(do_purge) {
    ...
    if (*buf) {
        ...
        if ((vict = get_char_vis(ch, buf, &number, FIND_CHAR_ROOM)) != NULL) {
            ...
            if (!IS_NPC(vict) && GET_LEVEL(ch) < ADMLVL_GOD) {
