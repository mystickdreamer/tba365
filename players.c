...
void build_player_index(void) {
    ...
    for (i = 0; i < rec_count; i++) {
        ...
        sscanf(line, "%ld %s %d %s %ld %d", &player_table[i].id, arg2,
                &player_table[i].level, bits, (long *) &player_table[i].last, &player_table[i].admlevel);
        ...
        while (get_line(fl, line)) {
            ...
            switch (*tag) {
                ...
                case 'S':
                    ...
                    else if (!strcmp(tag, "SkilXP")) load_skills_xp(fl, ch);
                    else if (!strcmp(tag, "Str ")) load_HMVS(ch, line, LOAD_STRENGTH);
                    break;cmp(tag, "ScrW")) GET_SCREEN_WIDTH(ch) = atoi(line);
                    else if (!strcmp(tag, "Skil")) lo
...
void save_char(struct char_data * ch) {
    ...
    if (GET_COND(ch, HUNGER) != PFDEF_HUNGER && GET_LEVEL(ch) < ADMLVL_IMMORT) fprintf(fl, "Hung: %d\n", GET_COND(ch, HUNGER));
    if (GET_COND(ch, THIRST) != PFDEF_THIRST && GET_LEVEL(ch) < ADMLVL_IMMORT) fprintf(fl, "Thir: %d\n", GET_COND(ch, THIRST));
    if (GET_COND(ch, DRUNK) != PFDEF_DRUNK && GET_LEVEL(ch) < ADMLVL_IMMORT) fprintf(fl, "Drnk: %d\n", GET_COND(ch, DRUNK));
    ...
    if (GET_PER(ch) != PFDEF_PER) fprintf(fl, "Per : %d\n", GET_PER(ch));
...
static void load_skills_xp(FILE *fl, struct char_data *ch) {
    int num = 0, num2;
    char line[MAX_INPUT_LENGTH + 1];
